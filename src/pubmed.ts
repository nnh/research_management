...
function getPubElement(
  ...
  if (!targetElement) {
    var elements = getElementsByTagName(root, "PubMedPubDate").filter(function (
      el
    ) {
...
function getRegisterdPubmedIds() {
  ...
  if (pubmedSheet === null) {
    ...
    pubmedSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
      "pubmedData"
...
function getUnregisteredPubmedData(
  registerdPubmedIds: any[],
  sheetPubmedIds: any[]
) {
  var pubmedSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    "pubmedData"
  ) as GoogleAppsScript.Spreadsheet.Sheet;
    ...
    if (registerdPubmedIds.indexOf(pubmedIds[i]) == -1) {
      ...
      if (root) {
        ...
        rowData[0] = [
          ...
          getTitle(root),
          getJournal(root),
          getAbstractText(root),
          getPubDate(root),
        ];
        pubmedSheet
          .getRange(pubmedSheet.getLastRow() + 1, 1, 1, 5)
          .setValues(rowData);
      }
    }
  }
