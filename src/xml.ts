...
export function filterElements(element: XML_Service.Element, predicate: (elm: XML_Service.Element, index: number, descendents: XML_Service.Content[]) => any): XML_Service.Element[] {
  const res = []
...
export function findElement(element: XML_Service.Element, predicate: (elm: XML_Service.Element, index: number, descendents: XML_Service.Content[]) => any): XML_Service.Element|undefined {
  const descendants = element.getDescendants()
  for (let i = 0; i < descendants.length; ++i) {
    if (descendants[i].getType() === XmlService.ContentTypes.ELEMENT) {
      const e = descendants[i].asElement()
