...
export function getHtmlRootElement(
  html: string
): domhandler.NodeWithChildren | undefined {
  const nodes = htmlparser2.parseDOM(html);
  const root = nodes.find(
    (node: domhandler.Node) => node instanceof domhandler.NodeWithChildren
  );
  return root as domhandler.NodeWithChildren | undefined;
}

export function getHtmlElementsByTagName(
  element: domhandler.NodeWithChildren,
  tagName: string
) {
...
export function getInnerText(element: domhandler.NodeWithChildren): string {
  const arr = element.children.map((child) => {
    if (
      child instanceof domhandler.NodeWithChildren ||
      child instanceof domhandler.Element
    ) {
...
export function getHtmlElementValue(
  target: domhandler.NodeWithChildren,
  name: string
) {
...
export function filterHtmlElements(
  element: domhandler.NodeWithChildren,
  predicate: (elm: domhandler.Element) => boolean
): domhandler.Element[] {
  const res = [];
